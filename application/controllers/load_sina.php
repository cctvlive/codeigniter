...
// 	echo '引入..........失败';
// }
class load extends  SAE_Controller
{
	function __construct()
	{
		...
		//define('WB_CALLBACK_URL' ,'http://phpoop.sinaapp.com/load/callback');
		}
		function index()
		{
	...
	public function qqcallback()
	{
        ...
        $to_mysql_key_id=$data['data']['data']['name'];
        $to_mysql_key=$data['data']['data'];
        $to_mysql=$data['qq'];
        //  print_r($to_mysql);
        ...
        $_SESSION=array('access_token_key'=>$data['data']['data']['openid'],'data'=>$data['data']['data']);
        // print_r($to_mysql['nickname']);
        // print_r($_SESSION['access_token_key']);
        // isset($username) && $username===$to_mysql['nickname'] /
         if( isset($_SESSION['access_token_key']) && $_SESSION['access_token_key']==@$access_token_key/*(是用户的唯一值) 等于 (匹配数据库里的openid字段里的值)*/)
          {
			/*,就输出成功/跳转到指定页面(个人信息页面或者是 想让授权者到的第一页面)*/
                	$select=array( 'access_token_key'=> $_SESSION['access_token_key']);
              $user_message['user_message']=$this->db->get_where('user',$select);
             // print_r($user_message['user_message']);
             foreach($user_message['user_message']->result() as $user_message){
                 // print_r($user_message->useradmin);
        ...
        //   print_r($to_mysql['sex']);
                ...
                'useradmin'=>$to_mysql['nickname'],/*名称*/
                 ...
                 'access_token_key'=>$to_mysql_key['openid'],/*绑定腾讯微博*/
                 'key_id'=>$to_mysql_key_id,/*腾讯key_id*/
                 'passwd'=>$to_mysql_key_id,/*腾讯用户密码*/
                 'insrt'=>date('Y-m-d H:i:s ',mktime()),/*插入时间 */
                 'email'=>'未填',
                 'mode'=>'0',
                 'status'=>'0',
                 'name'=>'腾讯会员','type'=>'2',/*类*/'group_id'=>'2'/*组*/);
             // if(isset($access_token_key)){echo "用户授权输出";}elseif( !isset($access_token_key)){echo "用户2次授权登陆输出";}
             if(isset($access_token_key)){ $this->db->insert('user',$data); }elseif(!isset($access_token_key) && $access_token_key===$_SESSION['access_token_key']){$user=array('access_token_key'=>$_SESSION['access_token_key']);$this->db->where($user); $this->db->update('user',$data);}
             
             
              if($this->db->affected_rows()>0){$select=array( 'access_token_key'=> $_SESSION['access_token_key']);
              $user_message['user_message']=$this->db->get_where('user',$select);
                  // print_r($user_message['user_message']);
             foreach($user_message['user_message']->result() as $user_message){
                 //print_r($user_message->useradmin);
